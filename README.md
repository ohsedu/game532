...
키보드·마우스·터치로 간단하게 즐기는 미니게임 6종. 점수를 기록하고 랭킹에 도전합니다.
...
| GAME 03 | **FACE OFF** · 방향 사수 | 적이 덮치는 순간 그 방향을 보고 있어야 산다. 8방향, 클러치 보너스 |
| GAME 04 | **STACK UP** · 블록 쌓기 | 흔들리는 블록을 타이밍 맞춰 쌓는다. 딱 맞추면 PERFECT |
| GAME 05 | **DASH RUN** · 점프 러너 | 점프·슬라이드·대시로 장애물을 돌파. 간격은 랜덤 |
| GAME 06 | **AIM LAB** · 타겟 사수 | 나타나는 타겟을 빠르고 정확하게 클릭. 빗맞히면 감점 |

조작은 게임마다 다릅니다. GAME 01–05 는 `↑ ↓ ← →` 와 `SPACE`(부스터 / 쌓기),
GAME 06 은 마우스입니다. 플레이 중 `ESC` 로 일시정지 / 재개할 수 있습니다.

**모바일**도 지원합니다. 게임 코드는 입력 장치를 모르고, 입력 계층이 터치를
같은 채널(가상 방향키 · 액션 · 포인터)로 변환합니다.

| 게임 | 터치 조작 |
| --- | --- |
| 총알 피하기 / 똥 피하기 | 화면을 끌어서 조준 (플로팅 조이스틱, 8방향) + BOOST 버튼 |
| 방향 사수 | 8방향 패드 |
| 블록 쌓기 | 화면 탭 |
| 점프 러너 | JUMP · SLIDE · DASH 버튼 |
| 타겟 사수 | 타겟을 탭 |
...
- **입력 채널은 셋.** 방향(4개 화살표) · 액션(SPACE) · 포인터. 물리 키보드와
  마우스와 터치가 같은 채널로 합류하므로 게임은 입력 장치를 구분하지 않습니다
  (`InputManager`의 virtual 채널, 조이스틱 기하는 `VirtualStick`).
